...
// 1. Extract Mock Data and Shared UI into src/shared.jsx
const sharedContent = `
    ...
    <>
      ...
      <aside className={\`fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-slate-200 bg-glass-light px-4 py-5 shadow-2xl shadow-slate-900/10 transition-transform duration-300 lg:sticky lg:top-0 lg:z-auto lg:h-screen lg:translate-x-0 lg:shadow-none \${isOpen ? 'translate-x-0' : '-translate-x-full'}\`}>
        ...
        <nav className="mt-8 space-y-1">
          {navigation.map((item) => (
            <a key={item.label} href="#" className={\`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition \${item.active ? 'bg-gradient-to-r from-slate-900 to-slate-800 shadow-lg text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}\`} onClick={onClose}>
...
fs.writeFileSync('./src/components/shared.jsx', sharedContent);

console.log('Created src/components/shared.jsx');
